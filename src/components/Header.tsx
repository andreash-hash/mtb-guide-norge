      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          <Link to="/" className="flex items-center gap-3 group">
            <img src={mtbSilhouette} alt="MTB Test Norge" className="h-14 w-14 object-contain transition-transform group-hover:scale-110" />
              ...
              {menuItems.map((item) =>
                item.href === "/#om-oss" ? (
                  <button
                    key={item.name}
                    onClick={() => handleNavClick(item.href)}
                    className="block w-full text-left px-3 py-2 text-foreground hover:text-accent hover:bg-accent/5 rounded-md transition-colors duration-200"
                  >
                    {item.name}
                  </button>
                ) : (
